...
//        performPresentAnimation()
		...
		// Arrows
		previousButton = UIBarButtonItem(customView: customToolbarButtonImage(
			UIImage.init(named: "left")!, imageSelected: UIImage.init(named: "left")!, action: #selector(gotoPreviousPage)))

		nextButton = UIBarButtonItem(customView: customToolbarButtonImage(UIImage.init(named: "right")!, imageSelected: UIImage.init(named: "right")!, action: #selector(gotoNextPage)))
	...
	open override func viewWillAppear(_ animated: Bool) {
		...
		if doneButtonImage == nil && doneButtonImageForeground == nil {
			...
			doneButton.setTitle(SSPhotoBrowserLocalizedStrings("Done"), for: .normal)
...
extension SSImageBrowser {
	...
	fileprivate func customToolbarButtonImage(_ image: UIImage, imageSelected selectedImage: UIImage, action: Selector) -> UIButton {
		...
		button.setImage(selectedImage, for: .disabled)
...
//			var finalImageViewFrame = CGRect(x: 0, y: y, width: screenWidth, height: h)
	...
	fileprivate func frameForDoneButtonAtOrientation(_ orientation: UIInterfaceOrientation) -> CGRect {
		...
		let screenWidth = screenBound.size.width
		return CGRect(x: screenWidth - 60, y: 50, width: 55, height: 26)
	}
